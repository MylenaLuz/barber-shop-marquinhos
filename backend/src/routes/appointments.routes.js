...
router.get("/slots", async (req, res) => {
  ...
  const existing = await Appointments.activeForBarberOnDate(barberId, date);
...
router.post("/", async (req, res) => {
  ...
  const service = await Services.getService(serviceId);
  if (!service) return res.status(404).json({ error: "Serviço não encontrado." });
  const barber = await Barbers.getBarber(barberId);
...
router.get("/", requireAuth, async (req, res) => {
  ...
  res.json(await Appointments.listAppointments({ date, dateFrom, dateTo, barberId, status }));
});
...
router.put("/:id", requireAuth, async (req, res) => {
  const current = await Appointments.getAppointment(req.params.id);
  ...
  if (patch.serviceId && patch.serviceId !== current.serviceId) {
    const service = await Services.getService(patch.serviceId);
    if (service) {
      patch.duration = service.duration;
      patch.price = service.price;
    }
  }
  const updated = await Appointments.updateAppointment(req.params.id, patch);
  res.json(updated);
});
...
router.post("/:id/complete", requireAuth, async (req, res) => {
  ...
  await Appointments.updateAppointment(req.params.id, { status: "concluido" });
  ...
  if (consumeCreditType && appt.clientId) {
    const active = await ClientPlans.getActiveForClient(appt.clientId);
    if (active) {
      try {
        await ClientPlans.consumeCredit(active.clientPlan.id, consumeCreditType, appt.id);
        return res.json({ ...(await Appointments.getAppointment(req.params.id)), creditConsumed: true });
      } catch (e) {
  ...
  const already = await Transactions.findByAppointment(appt.id);
  if (!already) {
    const service = await Services.getService(appt.serviceId);
  ...
  res.json(await Appointments.getAppointment(req.params.id));
});

// Protegido: cancelar
router.post("/:id/cancel", requireAuth, async (req, res) => {
  const appt = await Appointments.updateAppointment(req.params.id, { status: "cancelado" });
  if (!appt) return res.status(404).json({ error: "Agendamento não encontrado." });
  res.json(appt);
});
