...
router.get("/", requireAuth, async (req, res) => {
  res.json(await Plans.listPlans({ includeInactive: req.query.all === "1" }));
});
...
router.post("/", requireAuth, async (req, res) => {
  ...
  res.status(201).json(await Plans.createPlan({ name, price, validityDays, credits }));
});

router.put("/:id", requireAuth, async (req, res) => {
  const updated = await Plans.updatePlan(req.params.id, req.body || {});
  if (!updated) return res.status(404).json({ error: "Plano não encontrado." });
  res.json(updated);
});

router.delete("/:id", requireAuth, async (req, res) => {
  const result = await Plans.deletePlan(req.params.id);
  if (!result.ok) {
